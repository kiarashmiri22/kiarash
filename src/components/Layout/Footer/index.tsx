import React, { FC } from "react";
import Link from "next/link";
import { headerData } from "../Header/Navigation/menuData";
import { footerlabels } from "@/app/api/data";
import { Icon } from "@iconify/react";
import Logo from "../Header/Logo";

const socialIcons: Record<string, string> = {
  Email: "mdi:email-outline",
  GitHub: "fa6-brands:github",
  Telegram: "fa6-brands:telegram",
};

const Footer: FC = () => {
  return (
    <footer className="pt-16 bg-background">
      <div className="container px-4">
        <div className="grid grid-cols-1 sm:grid-cols-11 lg:gap-20 md:gap-6 sm:gap-12 gap-6  pb-16">
          <div className="lg:col-span-5 md:col-span-6 col-span-6 flex flex-col gap-6">
            <Logo />
            <p className="text-white/60 max-w-sm">
              Full-stack developer working across frontend, backend, and
              hardware. Always open to interesting projects.
            </p>
            <div className="flex gap-6 items-center relative z-1">
              {footerlabels.map((item, index) => (
                <Link href={item.herf} key={index} className="group">
                  <Icon
                    icon={socialIcons[item.label] ?? "lucide:link"}
                    width="24"
                    height="24"
                    className="text-white group-hover:text-primary"
                  />
                </Link>
              ))}
            </div>
          </div>
          <div className="lg:col-span-2 md:col-span-3 col-span-6">
            <h4 className="text-white mb-4 font-medium text-24">Links</h4>
            <ul>
              {headerData.map((item, index) => (
                <li key={index} className="pb-4">
                  <Link
                    href={item.href}
                    className="text-white/60 hover:text-primary text-17"
                  >
                    {item.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>
          <div className="lg:col-span-3 md:col-span-3 col-span-6">
            <h4 className="text-white mb-4 font-medium text-24">Contact</h4>
            <ul>
              {footerlabels.map((item, index) => (
                <li key={index} className="pb-4">
                  <Link
                    href={item.herf}
                    className="text-white/60 hover:text-primary text-17"
                  >
                    {item.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>
        </div>
        <div className="border-t border-white/10 py-6 text-center text-white/40 text-sm">
          © {new Date().getFullYear()} Kiarash Miri. All rights reserved.
        </div>
      </div>
    </footer>
  );
};

export default Footer;
